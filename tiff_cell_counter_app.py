...
def load_two_channel_tiff(path):
    ...
    if img.ndim != 3:
        raise ValueError(f"Expected a 3D TIFF with 2 channels, got shape {img.shape}")
    ...
    if img.shape[0] == 2:
        ...
        raise ValueError(f"Could not identify 2 channels in TIFF shape {img.shape}")
...
def analyze_tiff(tiff_path, method_name, params=None):
    ...
    yellow_over_total_count_pct = 100 * yellow_count / (yellow_count + red_count + green_count) if (yellow_count + red_count + green_count) > 0 else 0.0
    ...
    return {
        ...
        "Yellow over (Yellow+Red+Green) (%)": yellow_over_total_count_pct,
...
class CellCountingApp:
    def __init__(self, root):
        ...
        self.status_var = tk.StringVar(value="Select TIFF files or a folder, choose a method, then run.")

        top = ttk.Frame(root, padding=12)
        top.pack(fill="x")

        ttk.Label(top, text="Counting method:").grid(row=0, column=0, sticky="w", padx=(0, 8))
        ...
        ttk.Button(top, text="Select TIFF files", command=self.select_files).grid(row=0, column=2, padx=8)
        ttk.Button(top, text="Select folder", command=self.select_folder).grid(row=0, column=3, padx=8)
        ttk.Button(top, text="Clear selection", command=self.clear_selection).grid(row=0, column=4, padx=8)
    ...
    def select_folder(self):
        folder = filedialog.askdirectory(title="Select folder containing TIFF files")
        if not folder:
            return
        tif_files = sorted(glob.glob(os.path.join(folder, "*.tif"))) + sorted(glob.glob(os.path.join(folder, "*.tiff")))
        self.selected_files = tif_files
        self.refresh_file_list()
        self.status_var.set(f"{len(self.selected_files)} TIFF file(s) found in folder.")
    ...
    def run_analysis(self):
        ...
        for idx, path in enumerate(self.selected_files, start=1):
            try:
                ...
                self.log(f"Yellow over (Yellow+Red+Green): {result['Yellow over (Yellow+Red+Green) (%)']:.2f}%")
                self.log("-" * 50)
